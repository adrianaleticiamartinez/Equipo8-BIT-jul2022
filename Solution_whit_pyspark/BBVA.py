...
spark = SparkSession.builder.master("local[1]").appName("BBVA").getOrCreate()
...
def consulta(rol):
	try:
		datos = spark.read.csv("Datos_Descifrados.csv") #Colocar en la ruta correcta segun donde guarda el csv
		if(rol == "manager"):
			s = True
			while(s == True):
